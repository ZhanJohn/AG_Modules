 ...
 *******************************************************************************/
package com.ag.controls.pullrefreshview;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (a.hasValue(attrVal)) {
            mMode = Mode.mapIntToValue(a.getInteger(attrVal, 0));
        }
        ...
        int attrValAdapter = ResourceUtil.getStyleable(context,
                "PullToRefresh_ptrAdapterViewBackground");
        if (a.hasValue(attrVal)) {
            Drawable background = a.getDrawable(attrVal);
            if (null != background) {
                mRefreshableView.setBackgroundDrawable(background);
            }
        } else if (a.hasValue(attrValAdapter)) {
            Utils.warnDeprecation("ptrAdapterViewBackground",
                    "ptrRefreshableViewBackground");
            Drawable background = a.getDrawable(attrValAdapter);
            if (null != background) {
                mRefreshableView.setBackgroundDrawable(background);
            }
        }
        ...
        if (a.hasValue(attrVal)) {
            mOverScrollEnabled = a.getBoolean(attrVal, true);
        }

        attrVal = ResourceUtil.getStyleable(context,
                "PullToRefresh_ptrScrollingWhileRefreshingEnabled");
        if (a.hasValue(attrVal)) {
            mScrollingWhileRefreshingEnabled = a.getBoolean(attrVal, false);
        }
