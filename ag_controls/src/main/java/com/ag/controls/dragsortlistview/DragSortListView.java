...
package com.ag.controls.dragsortlistview;
    ...
    public DragSortListView(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = getContext().obtainStyledAttributes(attrs,
                    ResourceUtil.getStyleableArray(context, "DragSortListView"),
                    0, 0);

            mItemHeightCollapsed = Math.max(1, a.getDimensionPixelSize(
                    ResourceUtil.getStyleable(context, "DragSortListView_collapsed_height"),
                    1));

            mTrackDragSort = a.getBoolean(
                    ResourceUtil.getStyleable(context, "DragSortListView_track_drag_sort"),
                    false);
            ...
            mFloatAlpha = a.getFloat(
                    ResourceUtil.getStyleable(context, "DragSortListView_float_alpha"),
                    mFloatAlpha);
            mCurrFloatAlpha = mFloatAlpha;

            mDragEnabled = a.getBoolean(
                    ResourceUtil.getStyleable(context, "DragSortListView_drag_enabled"),
                    mDragEnabled);

            mSlideRegionFrac = Math.max(0.0f,
                    Math.min(1.0f, 1.0f - a.getFloat(
                            ResourceUtil.getStyleable(context, "DragSortListView_slide_shuffle_speed"),
                            0.75f)));

            mAnimate = mSlideRegionFrac > 0.0f;

            float frac = a.getFloat(
                    ResourceUtil.getStyleable(context, "DragSortListView_drag_scroll_start"),
                    mDragUpScrollStartFrac);

            setDragScrollStart(frac);

            mMaxScrollSpeed = a.getFloat(
                    ResourceUtil.getStyleable(context, "DragSortListView_max_drag_scroll_speed"),
                    mMaxScrollSpeed);

            removeAnimDuration = a.getInt(
                    ResourceUtil.getStyleable(context, "DragSortListView_remove_animation_duration"),
                    removeAnimDuration);

            dropAnimDuration = a.getInt(
                    ResourceUtil.getStyleable(context, "DragSortListView_drop_animation_duration"),
                    dropAnimDuration);

            boolean useDefault = a.getBoolean(
                    ResourceUtil.getStyleable(context, "DragSortListView_use_default_controller"),
                    true);

            if (useDefault) {
                boolean removeEnabled = a.getBoolean(
                        ResourceUtil.getStyleable(context, "DragSortListView_remove_enabled"),
                        false);
                int removeMode = a.getInt(
                        ResourceUtil.getStyleable(context, "DragSortListView_remove_mode"),
                        DragSortController.FLING_REMOVE);
                boolean sortEnabled = a.getBoolean(
                        ResourceUtil.getStyleable(context, "DragSortListView_sort_enabled"),
                        true);
                int dragInitMode = a.getInt(
                        ResourceUtil.getStyleable(context, "DragSortListView_drag_start_mode"),
                        DragSortController.ON_DOWN);
                int dragHandleId = a.getResourceId(
                        ResourceUtil.getStyleable(context, "DragSortListView_drag_handle_id"),
                        0);
                int flingHandleId = a.getResourceId(
                        ResourceUtil.getStyleable(context, "DragSortListView_fling_handle_id"),
                        0);
                int clickRemoveId = a.getResourceId(
                        ResourceUtil.getStyleable(context, "DragSortListView_click_remove_id"),
                        0);
                int bgColor = a.getColor(
                        ResourceUtil.getStyleable(context, "DragSortListView_float_background_color"),
                        Color.BLACK);
